...
int main() {
    ...
    char original_password[1001]; // Allow up to 1000 digits
    ...
    do {
        ...
        // Check if the password length is between 4 and 8 (inclusive) and the password is a valid number
        ...
        case 2: { // 2. 새로운 랜덤 비밀번호 받기
            ...
            if (password_length == 1 || password_length == 2 || password_length == 3) {
                if (password_length == 1) {
                    int new_password = generate_random_password(4);
                    printf("새로운 비밀번호 입니다: %04d\n", new_password);
                }
                else if (password_length == 2) {
                    int new_password = generate_random_password(8);
                    printf("새로운 비밀번호 입니다: %08d\n", new_password);
                }
                else if (password_length == 3) {
                    int new_password = generate_random_password(rand() % 5 + 4); // Random length between 4 and 8
